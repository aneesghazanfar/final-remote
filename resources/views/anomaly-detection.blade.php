<!DOCTYPE html>
<html>

<head>
    <meta charset="utf-8">
    <meta name="viewport"
        content="width=device-width, initial-scale=1, minimum-scale=1, maximum-scale=1, user-scalable=no, viewport-fit=cover">
    <title>Anomaly Detection Services &amp; Solutions &mdash; SSEDLAB</title>
    <script src="https://code.jquery.com/jquery-3.7.1.js"
        integrity="sha256-eKhayi8LEQwp4NKxN+CfCh+3qOVUtJn3QNZ0TciWLP4=" crossorigin="anonymous"></script>
    <link rel="stylesheet" href=" {{asset('css/main-css.css') }}">
    <link rel="apple-touch-icon" sizes="180x180" href=" {{asset('i/apple-touch-icon.png') }}">
    <link rel="icon" type="image/png" sizes="32x32" href=" {{asset('i/favicon-32x32.png') }}">
    <link rel="icon" type="image/png" sizes="16x16" href=" {{asset('i/favicon-16x16.png') }}">
    <link rel="manifest" href=" {{asset('i/site.webmanifest') }}">
    <link rel="mask-icon" href=" {{asset('i/safari-pinned-tab.svg') }}" color="#000">
    <link rel="shortcut icon" href=" {{asset('i/favicon.ico') }}">
    <meta name="description"
        content="Winfinity offers high-end, innovative, and regulated solutions for iGaming industry, providing premium quality content and products.">
    <meta name="msapplication-TileColor" content="#da532c">
    <meta name="msapplication-config" content=" {{asset('i/browserconfig.xml') }}">
    <meta name="theme-color" content="#ffffff">
    <style>
    .divlanding-list-item-text {
        position: static !important;
    }

    .delievery-industry-spec {
        width: 100%;
        /* Make the container take the full width of its parent */

        background-image: linear-gradient(to bottom, #000000a8, #00000070),
            url('https://itrexgroup.com/wp-content/uploads/2020/06/take-your-own-journey-scaled-e1626788633216-1440x449.jpg');
        background-repeat: no-repeat;

    }

    .divlanding-list-item-text {
        position: static !important;
    }

    .mb-sec-2-inner p {
        line-height: inherit !important;
    }

    .optimize-route-planning-container {
        position: relative;
        letter-spacing: inherit;
        line-height: inherit;
    }

    .optimize-route-planning-container {
        letter-spacing: inherit !important;
        line-height: inherit !important;
        margin-bottom: 20px;
    }

    .divlanding-list-item {
        width: auto !important;
        height: auto !important;
    }

    .divlanding-list-item-text {
        padding: 0.75rem;
        font-size: 16px;
    }

    .divlanding-list-item-text {
        max-width: inherit !important;
    }
    </style>
</head>

<body id="page-index">
    <noscript>
        <iframe src="https://www.googletagmanager.com/ns.html?id=GTM-PCK6J8B" height="0" width="0"
            style="display:none;visibility:hidden"></iframe>
    </noscript>
    <div class="body-wrap-1">
        @include("navbar")

        <div class="top_section-1 vis_on_scroll ">
            <div class="top_section vis_on_scroll banner-img-web">
                <div class="container-header">
                    <h1 class="banner-main-heading">
                        Anomaly detection
                        services & solutions
                    </h1>
                    <p class="tt-banner-para">
                        With the help of our anomaly detection development services, you can spot issues early on and
                        fix them to protect your processes, clients, revenue, and brand equity.


                    </p>


                    <section class="banner-btn">
                        <button class="tt-banner-btn">Partner with our anomaly detection</button>
                        <img src=" {{asset('img/SVG.svg') }}" alt="">
                    </section>

                </div>
            </div>
        </div>
        <!-- second Sections is here -->

        <section class="mb-sec-2">
            <div class="mb-sec-2-inner">
                <h2 class="mb-text-white">
                    Explore our anomaly detection services

                </h2>
                <p>
                    Our business is knowledgeable about the cloud, data science, artificial intelligence and its
                    subfields, and the Internet of Things. With this expertise, ITRex can assist you in developing an
                    anomaly detection system, whether it be at the edge or in the cloud. Decide on a strategy that will
                    therefore best serve you:


                </p>
                <div class="mb-row mb-sec-2-2">
                    <div class="mb-sec-2-inner-col">
                        <h3 class="sec-2-device">
                            Cloud anomaly detection

                        </h3>
                        <p>
                            We will gather data from various parts of your IT infrastructure, such as internal software
                            systems, linked assets and assets, and outside services, and transmit it to the cloud.
                            Custom-trained machine learning models will consume it there, look for any departures from a
                            predetermined baseline, and then urge you to take appropriate action.


                        </p>


                    </div>
                    <div class="mb-sec-2-inner-col">

                        <h3 class="sec-2-device">
                            Edge anomaly detection

                        </h3>
                        <p>
                            We'll set up anomaly detection systems to work their magic on the device to lower data
                            latency and cloud infrastructure costs in mission-critical systems, like production line
                            monitoring, autonomous vehicles, traffic management, and medical IoT solutions. The data
                            that doesn't need to be analyzed right now will be transferred in bulk to the cloud for
                            analysis and safe storage in the future.


                        </p>
                    </div>
                </div>
            </div>
            <br><br><br><br>
            <h3 class="sec-2-device">
                Our anomaly detection solutions will help you:
            </h3>
            <div class="mb-sec-2-inner">

                <div class="divlanding-list-list">
                    <div class="divlanding-list-col">
                        <div class="divlanding-list-item">
                            <div class="divlanding-list-item-icon"></div>
                            <div class="divlanding-list-item-text">
                                <div class="optimize-route-planning-container">
                                    <p class="optimize-route-planning-to-red">
                                        <b class="optimize-route-planning">
                                            Transform your separate IT systems and data sources into a single,
                                            integrated intelligent analytics solution.

                                        </b>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="divlanding-list-col">
                        <div class="divlanding-list-item">
                            <div class="divlanding-list-item-icon"></div>
                            <div class="divlanding-list-item-text">
                                <div class="optimize-route-planning-container">
                                    <p class="optimize-route-planning-to-red">
                                        <b class="optimize-route-planning">
                                            Real-time notifications will help you stay vigilant.

                                        </b>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div class="divlanding-list-list">
                <div class="divlanding-list-col">
                    <div class="divlanding-list-item">
                        <div class="divlanding-list-item-icon"></div>
                        <div class="divlanding-list-item-text">
                            <div class="optimize-route-planning-container">
                                <p class="optimize-route-planning-to-red">
                                    <b class="optimize-route-planning">
                                        Put an end to the requirement for human data inspection and, in the case of
                                        unsupervised machine learning, data labeling


                                    </b>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="divlanding-list-col">
                    <div class="divlanding-list-item">
                        <div class="divlanding-list-item-icon"></div>
                        <div class="divlanding-list-item-text">
                            <div class="optimize-route-planning-container">
                                <p class="optimize-route-planning-to-red">
                                    <b class="optimize-route-planning">
                                        Democratize access to data analytics so that everyone of your workers may use it
                                        to assist them make more informed decisions.

                                    </b>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div class="divlanding-list-list">
                <div class="divlanding-list-col">
                    <div class="divlanding-list-item">
                        <div class="divlanding-list-item-icon"></div>
                        <div class="divlanding-list-item-text">
                            <div class="optimize-route-planning-container">
                                <p class="optimize-route-planning-to-red">
                                    <b class="optimize-route-planning">Detect spikes, performance glitches,
                                        correlations, and trend shifts before they affect your business
                                    </b>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="divlanding-list-col">
                    <div class="divlanding-list-item">
                        <div class="divlanding-list-item-icon"></div>
                        <div class="divlanding-list-item-text">
                            <div class="optimize-route-planning-container">
                                <p class="optimize-route-planning-to-red">
                                    <b class="optimize-route-planning">Improve the security, reliability, and resilience
                                        of software and cyber-physical systems
                                    </b>
                            </div>
                        </div>
                    </div>
                </div>
            </div>




            <br><br><br>
            <section class="banner-btn mb-mt-50">
                <button class="tt-banner-btn">Build anomaly detection software with SSEDBAL
                </button>
            </section>
        </section>

        <section class="mb-container sec-3">
            <div class="delievery-industry-spec">
                <h2>
                    Our anomaly detection services

                    <span class="mb-orange"> <br>
                        tech stack
                    </span>

                </h2>
                <p>
                    Our anomaly detection company designs process monitoring and anomaly detection solutions
                    specifically for your business needs by combining open-source and proprietary technologies, such as
                    machine learning frameworks, data mining tools, and application integration services:
                </p>
            </div>
        </section>

        <br><br><br>


        <section class="mb-sec-2">
            <div class="mb-sec-2-inner">
                <div class="mb-row mb-sec-2-2">
                    <div class="mb-sec-2-inner-col">

                        <h3 class="sec-2-device">
                            Cloud services
                        </h3>
                        <p>
                            Google Cloud (Stream Analytics, AI services), AWS (Sagemaker, Cloud Watch, Kinesis,
                            Panorama, IoT Greengrass), Microsoft Azure (Cognitive Services, Anomaly Detector), and



                        </p>
                    </div>
                    <div class="mb-sec-2-inner-col">
                        <h3 class="sec-2-device">
                            Machine learning models
                        </h3>
                        <p>
                            Scikit-learn, PyTorch, TensorFlow, Keras, Apache Spark MLlib

                        </p>


                    </div>
                    <div class="mb-sec-2-inner-col">

                        <h3 class="sec-2-device">
                            Data integration, warehousing & analytics
                        </h3>
                        <p>
                            AWS Glue, PostgreSQL, MySQL, Apache Kafka, Amazon Kinesis, Trifacta, Apache NiFi, SQL Server
                            Integration Services, Azure Data Factory, and Amazon Kinesis
                        </p>


                    </div>
                    <div class="mb-sec-2-inner-col">

                        <h3 class="sec-2-device">
                            Data visualization
                        </h3>
                        <p>
                            Kibana, Grafana, D3.js, matplotlib, Plotly

                        </p>


                    </div>
                </div>
            </div>
            <br><br><br>
            <section class="banner-btn mb-mt-50">
                <button class="tt-banner-btn">Talk to our consultants</button>
            </section>
        </section>



        <section class="mb-container sec-3">
            <div class="delievery-industry-spec">
                <h2>
                    Industry-specific anomaly detection

                    <span class="mb-orange"> <br>
                        solutions we build
                    </span>

                </h2>
                <p>
                    We will acquire data you didn't know you had, distill it into actionable insights, and display the
                    results utilizing slick user interfaces, from cloud dashboards to voice assistants, whether you work
                    in supply chain management or oversee a BioTech lab. Our experience with anomaly detection software
                    includes:


                </p>
            </div>
        </section>

        <br><br><br>


        <section class="mb-sec-2">
            <div class="mb-sec-2-inner">
                <div class="mb-row mb-sec-2-2">
                    <div class="mb-sec-2-inner-col">

                        <h3 class="sec-2-device">
                            Healthcare
                        </h3>
                        <p>
                            Your healthcare, pharmaceutical, and life science technology systems will be set up such
                            that changes in patient vital signs data, adverse medication events, high-risk patients,
                            equipment breakdowns, and infectious disease outbreaks may all be detected automatically.



                        </p>
                    </div>
                    <div class="mb-sec-2-inner-col">
                        <h3 class="sec-2-device">
                            Manufacturing
                        </h3>
                        <p>
                            Our specialized anomaly solutions cover assembly verification, surface inspection, process
                            monitoring, and root cause investigation to help you improve equipment uptime, quality
                            assurance, energy efficiency, and accident avoidance.


                        </p>


                    </div>
                    <div class="mb-sec-2-inner-col">

                        <h3 class="sec-2-device">
                            Smart cities
                        </h3>
                        <p>
                            SSEDLAB will assist you in integrating anomaly detection tools into the technological
                            infrastructure of deployed smart cities. These include systems for animal control, energy
                            and waste management, public safety, and traffic monitoring using sensors and cameras.



                        </p>


                    </div>
                    <div class="mb-sec-2-inner-col">

                        <h3 class="sec-2-device">
                            Retail
                        </h3>
                        <p>
                            As an anomaly detection company, we assist retail and eCommerce businesses in a variety of
                            ways, including identifying credit card fraud, enhancing physical store security, analyzing
                            sudden drops and spikes in product demand, finding pricing mistakes, determining customer
                            satisfaction, and more!


                        </p>


                    </div>
                </div>
            </div>
            <br><br><br>
            <section class="banner-btn mb-mt-50">
                <button class="tt-banner-btn">Build an anomaly detection</button>
            </section>
        </section>
        <section class="mb-container sec-3">
            <div class="delievery-industry-spec">
                <h2>
                    Why select SSEDLAB among other

                    <span class="mb-orange"> <br>
                        anomaly detection companies?
                    </span>

                </h2>
            </div>
        </section>

        <br><br><br>

        <section class="mb-sec-2">
            <div class="mb-sec-2-inner">
                <div class="mb-row mb-sec-2-2">
                    <div class="mb-sec-2-inner-col">

                        <h3 class="sec-2-device">
                            We know your business inside and out.
                        </h3>
                        <p>
                            Due to our extensive experience in a variety of industries, such as healthcare, finance, and
                            commerce technology, we are able to completely comprehend the specific difficulties that
                            your company encounters and develop anomaly detection solutions that meet them.

                        </p>
                    </div>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

                    <div class="mb-sec-2-inner-col">
                        <h3 class="sec-2-device">
                            We breathe AI.
                        </h3>
                        <p>
                            Since before it became popular, SSEDLAB has been developing unique machine learning models,
                            ranging from supervised algorithms to deep neural networks. We are skilled in the collection
                            and preparation of training data as well as the development of ethical, precise, and ethical
                            AI-based anomaly detection solutions.

                        </p>


                    </div>
                </div>


            </div>
            <div class="mb-sec-2-inner">
                <div class="mb-row mb-sec-2-2">
                    <div class="mb-sec-2-inner-col">

                        <h3 class="sec-2-device">
                            We take the custom route.
                        </h3>
                        <p>
                            Based on reliable cloud services and open-source machine learning technology, our team
                            develops customized anomaly detection solutions. In contrast to SaaS-based tools, these
                            solutions can be customized, fully satisfy your needs, and scale as your business expands.
                        </p>
                    </div>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

                    <div class="mb-sec-2-inner-col">
                        <h3 class="sec-2-device">
                            We strive for your success
                        </h3>
                        <p>
                            Our anomaly detection service team won't stop until your linked palette changer or Holter
                            monitor successfully detects irregularities and alerts employees before accidents occur. Our
                            customer satisfaction score of 83% speaks for itself.

                        </p>


                    </div>
                </div>




            </div>
            <br><br><br>
            <section class="banner-btn mb-mt-50">
                <button class="tt-banner-btn">Talk to our consultants</button>
                <img src=" {{asset('img/SVG.svg') }}" alt="">
            </section>
        </section>
        <section class="mb-container sec-3">
            <div class="delievery-industry-spec-3">
                <h1>
                    Why SSEDLAB?
                </h1>
                <div class="delievery-industry-spec-3-row">
                    <div class="delievery-industry-spec-3-col">
                        <div class="mb-counter">
                            <h3 class="mb-counter-number">
                                13+
                            </h3>
                            <p class="mb-counter-text">
                                years of hands-on experience
                            </p>
                        </div>
                        <div class="mb-counter">
                            <h3 class="mb-counter-number">
                                200+
                            </h3>
                            <p class="mb-counter-text">
                                clients around the globe

                            </p>
                        </div>
                        <div class="mb-counter">
                            <h3 class="mb-counter-number">
                                600+
                            </h3>
                            <p class="mb-counter-text">
                                software products delivered

                            </p>
                        </div>
                    </div>
                    <div class="delievery-industry-spec-3-col">
                        <div class="mb-counter">
                            <h3 class="mb-counter-number">
                                300+
                            </h3>
                            <p class="mb-counter-text">
                                top-tier experts
                            </p>
                        </div>
                        <div class="mb-counter">
                            <h3 class="mb-counter-number">
                                3+
                            </h3>
                            <p class="mb-counter-text">
                                years' client engagement

                            </p>
                        </div>
                        <div class="mb-counter">
                            <h3 class="mb-counter-number">
                                90%
                            </h3>
                            <p class="mb-counter-text">
                                hold BS, MS or PhD in Math/Computer Science

                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </section>




        @include("latest-project")

        @include("contact-form")

        @include("footer")

        <div class="popup-custom">
            <div class="popup__wrapper popup__presentation">
                <div class="popup__close closePopup" data-popup="popup__presentation__close">
                    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M1 1L16.5 16.5" stroke="white" stroke-width="2"></path>
                        <path d="M16.5 1L1 16.5" stroke="white" stroke-width="2"></path>
                    </svg>
                </div>
                <div class="popup_presentation__wrap">
                    <div class="popup_presentation__content_wrap form_sucsess_parent">
                        <div class="popup_presentation__content_form">
                            <div class="section__head popup__small_head"><span class="color-small"><b
                                        class="top_section__head_i_inner">PRESENTATION</b> </span></div>
                            <div class="section__head small_head"><span class="color"><b
                                        class="top_section__head_i_inner">Download</b> </span><b
                                    class="top_section__head_i_inner">winfinity</b> <b
                                    class="top_section__head_i_inner">presentation</b></div>
                            <div class="section__bottom">Provide your Email and we will send you Winfinity product
                                Presentation download link.</div>
                            <div class="section__form">
                                <form class="form" action="/api/mailing/presentation-mail" method="post">
                                    <label class="form__input field">
                                        <input type="email" name="email" data-valid="false" data-name="email"
                                            placeholder="Email">
                                        <div class="form__input_label">Email</div>
                                        <div class="form__input_error" data-empty="The field is required"
                                            data-emptyall="all fields should be filled"
                                            data-error="invalid Email format" data-errorbox="invalid Email format"
                                            data-errorback="We can’t find this Email">
                                            <span>We can’t find this
                                                Email</span>
                                        </div>
                                    </label>
                                    <label class="form__check field">
                                        <input type="checkbox" data-valid="false" data-name="Agree"><span
                                            class="form__check_input"></span>
                                        <div class="form__check_label">By checking this checkbox, I give a permission
                                            for use of my personal data. </div>
                                    </label>
                                    <div class="form__submit">
                                        <button class="disabled button button--orange">
                                            <span class="button__wrap">
                                                <span class="button__inner_wrap">
                                                    <b class="button__inner_text">SEND ME A LINK</b>
                                                </span>
                                                <span class="button__arrow">
                                                    <svg class="alternative_solution" viewBox="0 0 1000 1000"
                                                        preserveAspectRatio="none">
                                                        <foreignObject class="foreignobject" width="1000" height="1000">
                                                            <span class="foreignobject"></span>
                                                        </foreignObject>
                                                    </svg>
                                                    <span class="button__arrow_svg">
                                                        <svg viewBox="0 0 17 19" fill="none"
                                                            xmlns="http://www.w3.org/2000/svg">
                                                            <path d="M6.36402 2L14 9.63598L6.36403 17.272"
                                                                stroke-width="3">
                                                            </path>
                                                            <path d="M14 9.63379L0.000713587 9.63379" stroke-width="3">
                                                            </path>
                                                        </svg>
                                                    </span>
                                                </span>
                                            </span>
                                        </button>
                                    </div>
                                </form>
                            </div>
                        </div>
                        <div class="inner_product_presentation__thanks inner_product_thanks">
                            <div class="inner_product_thanks__head"><span class="color">Thanks </span><br>for your
                                request</div>
                            <div class="inner_product_thanks__ckeck_email">Check your Email</div>
                            <div class="inner_product_thanks__email_to">Presentation Download Link was sent to <span
                                    class="mail_content">*****<a href="/cdn-cgi/l/email-protection" class="__cf_email__"
                                        data-cfemail="1a7b737d5a7d777b737634797577">[email&#160;protected]</a></span>
                            </div>
                        </div>
                    </div>
                    <div class="popup__presentation_img_wrap">
                        <img src="./public/img/main_page__download--mob.jpg" alt="">
                    </div>
                </div>
            </div>
        </div>
        <div class="popup">

        </div>
        <div class="popup__wrapper popup__cookies">
            <div class="popup__close closePopup">
                <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M1 1L16.5 16.5" stroke="white" stroke-width="2"></path>
                    <path d="M16.5 1L1 16.5" stroke="white" stroke-width="2"></path>
                </svg>
            </div>
            <div class="popup__title"> <span>We use cookies</span></div>
            <div class="popup__cookies_scroll">
                <div class="popup__description"> <span>This site uses cookies for several reasons. First and
                        foremost, to ensure the correct operation of the site, as well as to collect statistical
                        data and to personalize advertising and marketing content. Learn more about this on our <a
                            href="#">cookie policy page.</a></span></div>
                <div class="popup__settings">
                    <div class="popup__settings_wrapper">
                        <div class="popup__title"> <span>Necessary cookies</span></div>
                        <div class="popup__settings_orange"> <span>Always ON</span></div>
                        <div class="popup__description"> <span>the purpose of the use is to provide technical
                                functionality of our websites. The data from these cookies are processed on the
                                basis of Winfinity’s legitimate interest.</span></div>
                        <div class="popup__settings_checkboxes">
                            <div class="popup__settings_checkbox_container">
                                <div class="popup__title popup__settings_checkbox_label"> <span>Statistical
                                        cookies</span>
                                    <label class="popup__settings_checkbox">
                                        <input type="checkbox"><span> </span>
                                    </label>
                                    <div class="popup__settings_checkbox_arrow">
                                        <svg width="14" height="8" viewBox="0 0 14 8" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path d="M1 1L7 7L13 1" stroke="#60626A"></path>
                                        </svg>
                                    </div>
                                </div>
                                <div class="popup__settings_checkbox_info">
                                    <div class="popup__description popup__settings_checkbox_info_container">
                                        <span>the purpose of the use is to identify usage trends and derive
                                            analytics and statistics. These cookies are used to remember your
                                            choices and make assumptions about your preferences for statistical
                                            purposes, the legal basis is your consent. Winfinity uses an automated
                                            tool called Google Analytics which allows us to record and analyze how
                                            you and other visitors use the website. You can learn more about how
                                            Google Analytics works and the information it allows us to collect and
                                            analyze here <a
                                                href="https://support.google.com/analytics/answer/1012034?hl=lten&amp;ref_topic=6157800">https://support.google.com/analytics/answer/1012034?hl=lten&ref_topic=6157800.</a></span>
                                    </div>
                                </div>
                            </div>
                            <div class="popup__settings_checkbox_container">
                                <div class="popup__title popup__settings_checkbox_label"> <span>Marketing
                                        cookies</span>
                                    <label class="popup__settings_checkbox">
                                        <input type="checkbox"><span> </span>
                                    </label>
                                    <div class="popup__settings_checkbox_arrow">
                                        <svg width="14" height="8" viewBox="0 0 14 8" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path d="M1 1L7 7L13 1" stroke="#60626A"></path>
                                        </svg>
                                    </div>
                                </div>
                                <div class="popup__settings_checkbox_info">
                                    <div class="popup__description popup__settings_checkbox_info_container">
                                        <span>the purpose of the use is to identify usage trends and derive
                                            analytics and statistics. These cookies are used to remember your
                                            choices and make assumptions about your preferences for statistical
                                            purposes, the legal basis is your consent. Winfinity uses an automated
                                            tool called Google Analytics which allows us to record and analyze how
                                            you and other visitors use the website. You can learn more about how
                                            Google Analytics works and the information it allows us to collect and
                                            analyze here <a
                                                href="https://support.google.com/analytics/answer/1012034?hl=lten&amp;ref_topic=6157800">https://support.google.com/analytics/answer/1012034?hl=lten&ref_topic=6157800.</a></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="popup__cookies_buttons">
                <button class="button button--black_hover button--gradient_hover toggleSettings"
                    data-save="Save preferences" data-manage="manage cookies"><span class="button__wrap"><span
                            class="button__inner_wrap"><b class="button__inner_text">manage cookies</b></span><span
                            class="button__arrow">
                            <svg class="alternative_solution" viewBox="0 0 1000 1000" preserveAspectRatio="none">
                                <foreignobject class="foreignobject" width="1000" height="1000"><span
                                        class="foreignobject"></span></foreignobject>
                            </svg><span class="button__arrow_svg"><svg viewBox="0 0 17 19" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path d="M6.36402 2L14 9.63598L6.36403 17.272" stroke-width="3" />
                                    <path d="M14 9.63379L0.000713587 9.63379" stroke-width="3" />
                                </svg>
                            </span></span></span></button>
                <button class="button button--black_hover button--gradient_hover"><span class="button__wrap"><span
                            class="button__inner_wrap"><b class="button__inner_text">deny</b></span><span
                            class="button__arrow">
                            <svg class="alternative_solution" viewBox="0 0 1000 1000" preserveAspectRatio="none">
                                <foreignobject class="foreignobject" width="1000" height="1000"><span
                                        class="foreignobject"></span></foreignobject>
                            </svg><span class="button__arrow_svg"><svg viewBox="0 0 17 19" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path d="M6.36402 2L14 9.63598L6.36403 17.272" stroke-width="3" />
                                    <path d="M14 9.63379L0.000713587 9.63379" stroke-width="3" />
                                </svg>
                            </span></span></span></button>
                <button class="button button--orange popup__button_consent closePopup"><span class="button__wrap"><span
                            class="button__inner_wrap"><b class="button__inner_text">consent</b></span><span
                            class="button__arrow">
                            <svg class="alternative_solution" viewBox="0 0 1000 1000" preserveAspectRatio="none">
                                <foreignobject class="foreignobject" width="1000" height="1000"><span
                                        class="foreignobject"></span></foreignobject>
                            </svg><span class="button__arrow_svg"><svg viewBox="0 0 17 19" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path d="M6.36402 2L14 9.63598L6.36403 17.272" stroke-width="3" />
                                    <path d="M14 9.63379L0.000713587 9.63379" stroke-width="3" />
                                </svg>
                            </span></span></span></button>
            </div>
        </div>
    </div>
    </div>
    <script>
    function openCity(evt, cityName) {
        var i, tabcontent, tablinks;
        tabcontent = document.getElementsByClassName("tabcontent");
        for (i = 0; i < tabcontent.length; i++) {
            tabcontent[i].style.display = "none";
        }
        tablinks = document.getElementsByClassName("tablinks");
        for (i = 0; i < tablinks.length; i++) {
            tablinks[i].className = tablinks[i].className.replace(" active", "");
        }
        document.getElementById(cityName).style.display = "block";
        evt.currentTarget.className += " active";
    }
    </script>
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.6.4/jquery.min.js"></script>
    <!--    <script src="https://code.jquery.com/jquery-3.7.1.js" integrity="sha256-eKhayi8LEQwp4NKxN+CfCh+3qOVUtJn3QNZ0TciWLP4=" crossorigin="anonymous"></script>-->
    <script data-cfasync="false" src="{{ asset ('cdn-cgi/scripts/5c5dd728/cloudflare-static/email-decode.min.js') }}">
    </script>
    <script src="{{ asset ('js/tween.js') }}"></script>
    <script src="{{ asset ('js/custom.js') }}"></script>
    <script src="{{ asset ('js/axios.min.js') }}"></script>
    <script src="{{ asset ('js/swiper-bundle.min.js') }}"></script>
    <script src="{{ asset ('js/lightgallery.js') }}"></script>
    <script src="{{ asset ('js/main.min.js?v=5.0.6') }}"></script>
    <script src="{{ asset ('js/custom-main.min.js?v=1') }}"></script>
    <script src="{{ asset ('js/accordian.min.js') }}"></script>
</body>
<script>
function openCity(evt, cityName) {
    var i, tabcontent, tablinks;
    tabcontent = document.getElementsByClassName("mb-tabcontent");
    for (i = 0; i < tabcontent.length; i++) {
        tabcontent[i].style.display = "none";
    }
    tablinks = document.getElementsByClassName("mb-tablinks");
    for (i = 0; i < tablinks.length; i++) {
        tablinks[i].className = tablinks[i].className.replace(" active", "");
    }
    document.getElementById(cityName).style.display = "block";
    evt.currentTarget.className += " active";
}

const clickCarousel = (e, which) => {
    if (which == 'left') {
        leftIndicator.click()
    } else {
        rightIndicator.click()
    }

}
</script>

</html>